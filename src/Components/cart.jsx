...
function Cart() {
  ...
  const handleAddToCart = (product) => {
    if (product.data.variablePrice) {
      ...
      if (enteredPrice && !isNaN(enteredPrice) && enteredDescription) {
        addToCart({
          ...product,
          data: {
            ...product.data,
            price: parseFloat(enteredPrice),
            description: enteredDescription, // Adding the description here
          },
          quantity: 1,
        });
      } else {
        alert("Por favor, ingrese un precio y una descripción válidos.");
      }
    } else {
        ...
        if (item && item.data && item.data.Barcode) {
          ...
          if (product) {
            ...
            if (newStock >= 0) {
              ...
              showAlert(
                "danger",
                `No hay suficiente stock para ${product.data.title}`
              );
              setLoading(false);
              return;
            }
  ...
  const calculateTotal = useCallback(() => {
    const total = cart.reduce(
      (acc, item) => acc + (parseFloat(item.data.price) || 0) * item.quantity,
      0
    );
    setTotal(total);
  }, [cart]);
              ...
              <table className="cart-table">
                <thead className="cart-table-header">
                  <tr>
                    ...
                    <th>Precio</th>
                    <th>Cantidad</th>
                    <th>Descripción</th> {/* Add this header */}
                    <th>Acciones</th>
                  </tr>
                </thead>
                <tbody>
